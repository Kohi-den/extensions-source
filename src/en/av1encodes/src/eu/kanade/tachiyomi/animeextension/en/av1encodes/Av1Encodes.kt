    ...
    override suspend fun getVideoList(episode: SEpisode): List<Video> {
        return withContext(Dispatchers.IO) {
            try {
                ...
                if (!ddl.success) return@withContext emptyList()

                val videos = mutableListOf<Video>()

                val resLabel = Regex("""\[(\d+p)]""").find(ddl.fileName ?: "")?.groupValues?.get(1) ?: prefQuality
                val audioLabel = ddl.audioDetails?.audio
                    ?.mapNotNull { it.language }?.distinct()?.joinToString("/")
                    ?.let { " [$it]" } ?: ""

                val subtitleLabel = ddl.subtitles
                    ?.mapNotNull { it.language }?.distinct()?.joinToString("/")
                    ?.let { " [Subs: $it]" } ?: ""

                val sizeLabel = ddl.fileSize?.let { " · $it" } ?: ""
                val qualLabel = "AV1 · $resLabel$audioLabel$subtitleLabel$sizeLabel"

                ddl.streamLink?.takeIf { it.isNotBlank() }?.let { url ->
                    videos.add(Video(url, "$qualLabel · Stream", url))
                }
                ddl.downloadLink?.takeIf { it.isNotBlank() && it != ddl.streamLink }?.let { url ->
                    videos.add(Video(url, "$qualLabel · Direct DL", url))
                }
